...
namespace App\Filament\I2cAdmin\Resources\System;

use App\Enums\DefaultStatusEnum;
use App\Enums\ProfileInfos\SocialMediaEnum;
use App\Enums\ProfileInfos\UfEnum;
use App\Enums\TenantAccountRoleEnum;
use App\Filament\I2cAdmin\Resources\System\TenantAccountResource\Pages;
use App\Filament\I2cAdmin\Resources\System\TenantAccountResource\RelationManagers;
...
class TenantAccountResource extends Resource
{
        ...
        return Forms\Components\Section::make(__('Acesso ao Sistema'))
            ...
            ->schema([
                Forms\Components\TextInput::make('user.email')
                    ...
                    ->live(debounce: 1000)
                    ->afterStateUpdated(
                        function (UserService $service, ?string $state, callable $set): void {
                            $user = $service->getUserByEmail(email: $state);
                    ...
                    Tables\Actions\ActionGroup::make([
                        Tables\Actions\ViewAction::make()
                            ->extraModalFooterActions([
                                ...
                                Tables\Actions\Action::make('panel')
                                    ->label(__('Visualizar Painel'))
                                    ->button()
                                    ->url(
                                        fn(TenantAccount $record): string =>
                                        route('filament.admin.pages.dashboard', ['tenant' => $record]),
                                    )
                                    ->openUrlInNewTab(),
                            ]),
        ...
        return [
            ...
            Tables\Columns\TextColumn::make('name')
                ...
                ->searchable(
                    ...
                    $service->tableSearchByNameAndCnpj(query: $query, search: $search)
                )
                ->sortable(),
    ...
    protected static function getTableFilters(): array
    {
        return [
            ...
            //     ->label(__('Tipo(s)'))
            //     ->multiple()
            //     ->options(TenantAccountRoleEnum::class),
            Tables\Filters\SelectFilter::make('owner')
                ->label(__('Titular(es) da(s) conta(s)'))
                ->relationship(
                    name: 'owner',
                    titleAttribute: 'name'
                )
